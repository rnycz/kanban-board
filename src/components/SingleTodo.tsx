...
import { Todo, colors, editTaskOn, taskDeleted, markedTask, successEdit } from "../model";
...
const SingleTodo: React.FC<Props> = ({index, todo, todos, setTodos}: Props) => {
    ...
    const handleEdit = (e: React.FormEvent, id: number) =>{
        ...
        setEdit(false)
        if(todo.todo !== editTodo || todo.color !== editColor){
            successEdit()
        }
    }
                ...
                <form className={`single-todo ${snapshot.isDragging ? "drag-form" : ""}`} 
                    ...
                    {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef}>
                        <div className="time-container">
                            <span className="date"><GoDiffAdded className="date-icon" />{todo.addDate}</span>
                            <span className="date"><FcExpired className="date-icon" />{todo.finishDate}</span>
                        </div>
                        <hr />
                        <div>
                            {
                                edit ? (
                                    <div className="single-todo-container">
                                        <div className="single-todo-color-edit" onClick={() => setShowColorPicker(!showColorPicker)}
                                            ...
                                            {
                                                showColorPicker ? <CirclePicker className="input-color" colors={colors}
